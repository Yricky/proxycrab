...
Export selected successful HTTP captures, or every eligible capture in a Session:

```bash
node <skill-dir>/scripts/har-export.mjs \
  --session-id 3 \
  --log-ids 1041,1042 \
  --output /tmp/proxycrab-selected.har

node <skill-dir>/scripts/har-export.mjs \
  --session-id 3 \
  --output /tmp/proxycrab-session.har
```

Omit `--session-id` to use the active Session. The complete HAR is generated by ProxyCrab; the
script writes it without parsing or reformatting it. Only successful captures with responses are
included, and the synthetic TLS CONNECT record is excluded. The existing `log-export.mjs` remains
the single-capture raw ProxyCrab JSON diagnostic export.

HAR export does not redact captured data. The artifact can contain Authorization headers, cookies,
credentials, and full request/response bodies. Do not print its contents or share it beyond the
user's requested destination; report only the saved path and requested selection unless the user
