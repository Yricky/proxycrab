...
const mediaType = computed(() => {
  ...
  return loaded.value
    ? (sniffMediaType(loaded.value.bytes) ?? declared)
    : declared;
});
...
const previewKind = computed(() => {
  ...
  if (media === "application/grpc" || media === "application/grpc+proto")
    return "grpc";
  if (media.includes("protobuf") || media === "application/x-protobuf")
    return "protobuf";
  if (
...
const editorValue = computed(() => {
  if (previewKind.value === "protobuf" || previewKind.value === "grpc")
    return structured.value;
  const text = decodedText.value;
  if (
    pretty.value &&
    (mediaType.value.includes("json") || /^[\s]*[\[{]/.test(text))
  ) {
    try {
...
const editorLanguage = computed(() => {
  const media = mediaType.value;
  if (
    previewKind.value === "protobuf" ||
    previewKind.value === "grpc" ||
    media.includes("json")
  )
    return "json";
  if (media.includes("html")) return "html";
  if (media.includes("xml") || media === "image/svg+xml") return "xml";
  if (media.includes("javascript") || media.includes("ecmascript"))
    return "javascript";
...
watch(
  () => [
    props.target.kind,
    props.target.id,
    props.target.kind === "interceptor" ? props.target.executionId : undefined,
    props.target.kind !== "breakpoint" ? props.target.sessionId : undefined,
    props.side,
    props.body.type,
    storedSize.value,
    path.value,
  ...
  <section class="body-viewer">
    <div class="body-head">
      ...
      <span class="body-actions">
        ...
        <button
          v-if="
            path && (error || previewKind === 'unsupported' || !canRequestFull)
          "
          class="btn"
...
.media-preview {
  ...
  overflow: auto;
  padding: 12px;
  background: color-mix(in srgb, var(--bg-app) 75%, #000);
}
.media-preview img,
.media-preview video {
  max-width: 100%;
